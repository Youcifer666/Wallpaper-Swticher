...
Ported verbatim from amnweb/yasb (src/core/widgets/services/wallpapers/
engine.py) for the Youcifer Wallpaper switcher, with three small changes:
the win32con constants are inlined (no pywin32 dependency), a Python-2 style
`except` in _free_resources was fixed, and WallpaperEngine accepts an explicit
fit `style` so the overlay layout can match the style the caller is about to
apply rather than the one currently in the registry.
...
# win32con constants (defined here so pywin32 is not required)
GWL_EXSTYLE = -20
GWL_STYLE = -16
...
# Valid DESKTOP_WALLPAPER_POSITION values (see wallpaper_api.POSITIONS)
POSITIONS_SET = {"center", "tile", "stretch", "fit", "fill", "span"}
...
class WallpaperEngine(QWidget):
    ...
    def __init__(self, image_path: str, animation: str = "circle",
        ...
        # Adaptation for Youcifer Wallpaper: the caller knows the fit style it
        # is about to apply (the overlay layout must match what Windows will
        # settle on AFTER the commit, not the mode currently in the registry).
        self.fit_mode = style if style in POSITIONS_SET else _read_fit_mode()
